...
class AuthApi {
  ...
  static register(
    ...
    log("URL ${NetworkConstantsUtil.register}");
    log("FCM ${fcmToken} -- ${voipToken}");
    await ApiWrapper()
      ...
      if (response?.success == true) {
        ...
        AppUtil.showToast(
            message: response.message ?? successString.tr, isSuccess: true);
        successCallback(token);
      } else {
        log('ERROR ${response?.message ?? errorMessageString.tr}');
        AppUtil.showToast(
            message: response?.message ?? errorMessageString.tr,
            isSuccess: false);
      }
