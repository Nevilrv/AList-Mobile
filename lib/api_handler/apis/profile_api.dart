...
class ProfileApi {
  ...
  static postRelationshipSettings(
      {required int relationSetting,
      required VoidCallback resultCallback}) async {
    var url = NetworkConstantsUtil.postRelationshipSetting;
    await ApiWrapper().postApi(url: url, param: {
  ...
  static getMyRelations(
      {required Function(List<MyRelationsModel>) resultCallback}) async {
    var url = NetworkConstantsUtil.myRelations;
  ...
  static getRelationshipNames(
      {required Function(List<RelationshipName>) resultCallback}) async {
    var url = NetworkConstantsUtil.relationshipNames;

    await ApiWrapper().getApi(url: url).then((result) {
      if (result?.success == true) {
        var items = result!.data['relations'];
        resultCallback(List<RelationshipName>.from(
            items.map((x) => RelationshipName.fromJson(x))));
      }
    });
  }

  static getUsersRelationships(
      {required int userId,
      required Function(List<MyRelationsModel>) resultCallback}) async {
    var url =
        '${NetworkConstantsUtil.getRelationbyUser}?user_id=$userId&expand=user,realationShip';

    await ApiWrapper().getApi(url: url).then((result) {
      if (result?.success == true) {
        var items = result!.data['relations']['items'];
        resultCallback(List<MyRelationsModel>.from(
            items.map((x) => MyRelationsModel.fromJson(x))));
      }
    });
  ...
  static getMyInvitations(
      {required Function(List<MyInvitationsModel>, APIMetaData)
          resultCallback}) async {
    var url = NetworkConstantsUtil.myInvitations;

    await ApiWrapper().getApi(url: url).then((result) {
      if (result?.success == true) {
        var items = result!.data['invitation']['items'];
        resultCallback(
            List<MyInvitationsModel>.from(
